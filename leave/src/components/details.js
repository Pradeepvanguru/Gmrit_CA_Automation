...
function Home() {
  ...
  const fetchUserDetails = async (acceptedBy) => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/registers/${acceptedBy}`);
  ...
  const fetchAcceptedRequests = async () => {
    setLoading(true);
    try {
      const response = await axios.get('${process.env.REACT_APP_BACKEND_URL}/api/leave');
