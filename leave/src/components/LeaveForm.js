...
function LeaveForm() {
  ...
  const onSubmit = (data, event) => {
    ...
    axios.post('${process.env.REACT_APP_BACKEND_URL}/api/leave', data)
      .then((response) => {
  ...
  const fetchFacultyData = async ({ email, branch, sem, year }) => {
    try {
      const response = await axios.post('${process.env.REACT_APP_BACKEND_URL}/api/getFacultyData', { email, branch, sem, year });
      if (response.data) {
